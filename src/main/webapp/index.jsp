					...
					<form method="post">
						<div class="modal-body">
							...
							<table class="table">
								...
								<tbody>
									<tr>
										<td>${requestScope.saving.savingsId}</td>
										<td>${requestScope.saving.savingsInterest}</td>
										<td>${requestScope.saving.savingsStartDate }</td>
										<td>${requestScope.saving.savingsMaturityDay }</td>
										<td>$${requestScope.saving.savingsAmount }</td>										
									</tr>		
								</tbody>
							</table>
							<h6>Starting Amount: $${requestScope.saving.savingsAmount}</h6>
							<h6>Current Amount (If you take out now): $${requestScope.saving.currentAmount() }</h6>
							<h6>Estimated Final Amount: $${requestScope.saving.finalAmount() }</h6>
							<strong>Here is your Total Savings progress</strong><br>
							<small>Keep
							...
							<div class="progress">
								<div
									...
									aria-valuemax="100" style="width: ${requestScope.saving.getPercentageToVip(account)}%"></div>
							</div>
							<small>Your progress: ${requestScope.account.accountTotalSavings } / 100.000$</small>
						</div>
						<div class="modal-footer">
							<c:if test="${not empty requestScope.saving.savingsId }">
							<button type="submit" name="action" value="reqTakeSaving"
								class="btn btn-success" data-dismiss="modal">Take out Your Current Saving</button>
							</c:if>
							<c:if test="${empty requestScope.saving.savingsId }">
							<button type="submit" name="action" value="reqTakeSaving"
								class="btn btn-success" data-dismiss="modal" disabled>Take out Your Current Saving</button>
							</c:if>
						</div>
					</form>
				</div>
