...
public class CustomerDAO_Impl {
	...
	public void performTakeOutSaving(Savings s) {
		...
		try {
			...
			statement.setDouble(1, s.currentAmount());
			statement.setString(2, s.getSavingsOwner());
			statement.executeUpdate();
		} catch (SQLException e) {
	...
	public void performSubtract(Savings s) {
		...
		try {
			PreparedStatement statement = connection.prepareStatement(subtractSql);
			statement.setDouble(1, s.getSavingsAmount());
			statement.setString(2, s.getSavingsOwner());
			statement.executeUpdate();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
		...
		System.out.println("subtract performed");
	}
...
//            statement.setDate(6, bill.getBillPaidDate());
	...
	public String getCustomerIdByUsername(String username) {
		...
		String query = "SELECT customer_id FROM Customer WHERE customer_username = ?";
	...
	public ArrayList<Savings> getCurrentCustomerSaving(Customer c) {
		ArrayList<Savings> listSavings = new ArrayList<Savings>();
		
		String sql = "SELECT * FROM Customer inner join Savings on Customer.customer_id = Savings.savings_owner where customer_id = ? AND savings_inProcess = 'true'";
		ResultSet rs = null;
		try {
			PreparedStatement statement = connection.prepareStatement(sql);
		...
		try {
			while(rs.next()) {
				...
				s.setSavingsId(rs.getString("savings_id"));
				s.setSavingsInterest(Double.parseDouble(rs.getString("savings_interest")));
				s.setSavingsMaturityDay(rs.getDate("savings_maturityDay"));
				s.setSavingsOwner(rs.getString("savings_owner"));
				s.setSavingsAmount(Double.parseDouble(rs.getString("savings_amount")));
				s.setSavingsStartDate(rs.getDate("savings_startDate"));
				listSavings.add(s);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
		...
		System.out.println(listSavings);
		return listSavings;
	}
	
	public Date monthsFromNow(Date startDate, String months) {
		Date finalDate = null;
		LocalDate startDateLocal = startDate.toLocalDate();
		switch(months) {
		case "3":
			finalDate = Date.valueOf(startDateLocal.plusMonths(3));
			break;
		case "6":
			finalDate = Date.valueOf(startDateLocal.plusMonths(6));
			break;
		case "9":
			finalDate = Date.valueOf(startDateLocal.plusMonths(9));
			break;
			default: break;
		}
	...
	public Savings getSavingsById(String savingsId) throws SQLException {
        String sql = "SELECT * FROM Savings WHERE savings_id = ?";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setString(1, savingsId);
            try (ResultSet resultSet = statement.executeQuery()) {
                if (resultSet.next()) {
                    Savings savings = new Savings();
                    savings.setSavingsId(resultSet.getString("savings_id"));
                    savings.setSavingsInterest(resultSet.getDouble("savings_interest"));
                    savings.setSavingsMaturityDay(resultSet.getDate("savings_maturityDay"));
                    savings.setSavingsOwner(resultSet.getString("savings_owner"));
                    savings.setSavingsAmount(resultSet.getDouble("savings_amount"));
                    savings.setSavingsStartDate(resultSet.getDate("savings_startDate"));
                    savings.setInProcess(resultSet.getBoolean("savings_inProcess"));
                    return savings;
                } else {
                    return null; // Savings not found
                }
            }
        }
    }
	
	public String getAccountNumberById(String customerId) {
		String accNum = null;
		String sql = "SELECT customer_bankAccount from Customer where customer_id = ?";
		ResultSet rs = null;
		try {
			PreparedStatement statement = connection.prepareStatement(sql);
		...
		try {
			while (rs.next()) {
				accNum = rs.getString("customer_bankAccount");
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
